...
# Simulate our neuron using a while loop.
...
# Initialise and execute the "neuron" that checks for spikes and prints them if a threshold is reached
# Simulate our neuron using a for-loop.
nQ5 = Neuron(Vmem = -72, go = True)
spike = False
steps = np.linspace(0, 5.0, 50)
for t in steps:
	spike = nQ5.checkSpike()
	nQ5.increasePotential(2)
	if spike == True:
		print(f"Spike == {spike}")
	if spike == False:
		pass
